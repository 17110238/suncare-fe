...
import {
  ...
  handleConfirmAndPaymentPatient,
...
=======
  handleConfirmSchedule = async (item, action) => {
    ...
    let data = {
      ...
      price:
        language === "vi"
          ? item.priceDataPatient.valueVi
          : item.priceDataPatient.valueEn,
      patientName: item?.patientData?.firstName,
      phoneNumber: item?.patientData?.phoneNumber,
      currentDate: this.state.currentDate,
      doctorName:
    ...
    if (res?.errCode === 0) {
      toast.success(res?.errMessage);
>>>>>>> e40ce5398ad24b57e6237a1f30384d126fae1583
    }
  };
        ...
        <div className="grid grid-cols-3">
          <div>
            <label className="font-bold">
              {" "}
              <FormattedMessage id="manage-schudule.choose-date" />
            </label>
            <DatePicker
              ...
              {dataPatient.length > 0 &&
                dataPatient.map((item, index) => (
                  <tr className="group" key={item.id}>
                    ...
                    <td className="whitespace-nowrap group-hover:bg-gray-50 py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6 ">
                      {Object.keys(item?.patientData).length > 0
                    ...
                    <td className="whitespace-nowrap group-hover:bg-gray-50 py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6 ">
                      {language === "vi" ? (
                        <NumberFormat
                          value={item?.priceDataPatient?.valueVi}
                          displayType={"text"}
                          thousandSeparator={"."}
                          decimalSeparator={","}
                          suffix={" VND"}
...
=======
                        ...
                        <NumberFormat
                          value={item?.priceDataPatient?.valueEn}
                          displayType={"text"}
                          thousandSeparator={"."}
                          decimalSeparator={","}
                          suffix={" USD"}
                        ...
                        >
                          <Menu.Items className="absolute border-2 border-gray-200 z-10 flex flex-col top-7 -right-6 w-56 h-24 mt-2 origin-top-right bg-white rounded-md shadow-lg">
                            <div className="flex z-10 w-full justify-around items-center text-white">
                              <div className="flex flex-col">
                                <span
                                  className="text-green-500 mt-2 flex cursor-pointer items-center text-sm "
                                  onClick={(e) =>
                                    this.handleConfirmSchedule(item, "confirm")
                                  }
                                >
                                  {" "}
                                  <FaCheckCircle className="text-xl mr-2" /> Xác
                                  nhận và gửi thanh toán
                                </span>
                                <span
                                  className="text-yellow-500 mt-2 flex cursor-pointer items-center text-sm "
                                  onClick={(e) =>
                                ...
                                >
                                  {" "}
                                  <FaTimes className="text-xl mr-2" /> Không
                                  khám
                                </span>
                                <span
                                  className="text-red-500 mt-2 flex cursor-pointer items-center text-sm "
                                  onClick={(e) =>
                                    this.handleConfirmSchedule(item, "cancel")
                                  }
                                >
                                  {" "}
                                  <FaWindowClose className="text-xl mr-2" /> Hủy
                                  lịch
                                </span>
                              </div>
                            </div>
