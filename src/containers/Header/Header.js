  ...
  render() {
    ...
    return (
      <div className="flex justify-between items-center bg-indigo-600 text-gray-200 z-50">
        {/* thanh navigator */}
        <div className="header-tabs-container z-50 flex item-center">
        <div className="flex items-center" style={{marginRight: "10px", cursor:"pointer"}} onClick={() => this.returnToHome()}>
          <span className="ml-2 text-white-600 font-semibold">
            <FormattedMessage id="homeheader.home-page" />
          </span>
        </div>
          ...
          <div className="flex justify-between items-center">
            <div className="text-white mr-3">
              ...
              <span className="text-white">
                {language === "vi" ? userInfo?.lastName : userInfo.firstName} !
              </span>
            </div>
            {language === "vi" ? (
              <img className="w-8" src={vi} />
            ) : (
              <img className="w-8  " src={en} />
            )}
            <select
              value={language}
              className="outline-none mr-4 cursor-pointer ml-2"
